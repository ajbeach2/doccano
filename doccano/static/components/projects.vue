                    ...
                    tbody
                      tr(v-for="project in selectedProjects", v-bind:key="project.id")
                          ...
                          div.dataset-item__main.is-vertical
                            div.dataset-item__main-title
                              div.dataset-item__main-title-link.dataset-item__link
                                a.has-text-black(v-bind:href="'/doccano/projects/' + project.id")
                                  | {{ project.name }}
                        ...
                        td.is-vertical(v-if="isProjectAdmin.get(project.id)")
                          a(v-bind:href="'/doccano/projects/' + project.id + '/docs'") Edit
...
export default {
  ...
  created() {
    Promise.all([
      defaultHttpClient.get('/doccano/v1/projects'),
      defaultHttpClient.get('/doccano/v1/me'),
    ]).then(([projects, me]) => {
  ...
  methods: {
    deleteProject() {
      defaultHttpClient.delete(`/doccano/v1/projects/${this.project.id}`).then(() => {
        this.isDelete = false;
    ...
    create() {
      ...
      defaultHttpClient.post('/doccano/v1/projects', payload)
        .then((response) => {
          window.location = `/doccano/projects/${response.data.id}/docs/create`;
        })
        .catch((error) => {
